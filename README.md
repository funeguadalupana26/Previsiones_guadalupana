...
Bronce, Plata, Oro y Platino ya están cargados en `CONFIG.PLANES['Inhumación']`.
El **Paquete Platino** no tiene precio fijo en el código porque su costo
cambia en cada contrato (ataúd elegido, autobús, sala de velación, etc.):
el precio base y los productos adicionales se capturan directamente en la
app al armar cada contrato, no se editan en `index.html`.

El contrato en PDF incluye la Cláusula Tercera (días y fechas de pago:
solo del 1 al 5 y del 16 al 20 de cada mes) y la Cláusula Novena, que
distingue cancelación voluntaria (reembolso con deducción del 20%) de
cancelación por falta de pago de dos mensualidades/quincenas consecutivas
...
La pantalla de login y la barra superior de la app muestran el logo real
de Funerales Guadalupana (`logo.png`). Si cambias el logo, reemplaza ese
archivo con uno de proporciones similares (fondo transparente recomendado).
